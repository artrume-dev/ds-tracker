...
router.get('/status', async (req, res) => {
    try {
        const status = EmailService_1.emailService.getStatus();
        ...
        if (emailConfigData && status.status === 'not_initialized') {
            ...
            await EmailService_1.emailService.initialize(emailConfig);
            // Get updated status
            const updatedStatus = EmailService_1.emailService.getStatus();
            return res.json({
                success: true,
                data: updatedStatus
            });
...
router.post('/configure', async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: {
                message: 'Email configuration saved successfully',
                status: EmailService_1.emailService.getStatus()
            }
        });
    }
    catch (error) {
